import re,os,sys,cgi,math,shutil, glob, zipfile, datetime
...
# カレントディレクトリに存在するか確認
if not os.path.exists("./" + file_grd):
    # ホームディレクトリにあるか確認（~ を展開）
    file_grd_home = os.path.expanduser(f"~/{file_grd}")
    if not os.path.exists(file_grd_home):
        print(f"Error: The file '{file_grd}' was not found in the current directory or your home directory. Execution stopped.")
        exit()
    else:
...
#<head>
#    <meta charset="UTF-8">
#        <title>Ocean eDNA map：TITLE</title>
#            <style type="text/css">
#               body { font-size: 130%; }
...
def check_and_read_csv(file_path, index_col=0):
    #print("### check_and_read_csv() ###<br>")
    if os.path.exists(file_path):
        try:
            df_reads_fn = pd.read_csv(file_path)
            return df_reads_fn
        except Exception as e:
            print(f"Error1 in csv: Cannot find csv file, {input_file_userDB.filename}.<br>")
            exit()
            return None
    else:
        print(f"Error2 in csv: Cannot find csv file, {input_file_userDB.filename}.<br>")
        exit()
        return None
