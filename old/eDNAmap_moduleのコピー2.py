...
def get_parameters_from_flask_request():
    ...
    #print("input_file_userDB.file:", input_file_userDB.file)
    print("### Exit point module.py 60 ###")
    #exit()
    ...
    if not list_cruiseNames:
        error_msg = "Error: Please upload a .xlsx/.csv/.txt file or select cruise/project names."
    ...
    for rec in list_dfs_cruises:
        ...
        for sample_id in df_reads.columns:
            ...
            for index, row in df_reads_only2.iterrows():
                reads = row.iloc[0]
                species = row.iloc[1]
                name_hit_detected = dic_scname2commonName.get(species, "NA").rstrip("\n")
                #print(f"read: {reads}, species: {species}")
                #if reads > 0:
                #    detected_species.append({
                #        "name": species,
                #        "common_name": name_hit_detected,
                #        "reads": int(reads)
                #    })
                if reads > 0:
