from flask import Flask, session, render_template, request, jsonify, send_from_directory, redirect
from flask_session import Session
from utils.session_utils import save_params_to_session
from datetime import timedelta
from eDNAmap_module import (
    run_analysis,
    load_oednamap_results,
...
def submit():
    ...
    #print("ASV_comparison_criateria:", ASV_comparison_criateria)
    ...
    try:
        list_dfs_cruises, list_dfs_cruises_stations, list_dfs_cruises_depths = prepare_fundamental_dataframes(eachDirAddress)
        ...
        print("Error generating dataframes 1:", e)
        return jsonify({'error': f'Failed to generate dataframes: {e}'}), 500
    # セッションの中身を確認
    ...
    # HTML結果の構築と保存
    result_html = build_result_html(dirname_rand, eachDirAddress, list_dfs_cruises)
